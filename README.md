...
The compatible candidate set is `company-product-knowledge-builder 0.1.0`, `industry-application-map-builder 0.4.0-beta.6`, `foreign-trade-customer-development 0.2.0-beta.2`, `foreign-trade-customer-operations 0.2.0-beta.2`, and `foreign-trade-workflow-director 0.3.0-beta.4`. Do not install the candidate industry plugin over an active `0.4.0-beta.5` R4 run; close that frozen test cycle on its existing source and artifacts first. Marketplace discovery, plugin installation, and target-environment workflow audit remain separate checks.
