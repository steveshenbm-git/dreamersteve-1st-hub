...
10. **Three-factor solution search and provisional whole:** For an unbounded, high-impact, meaning-changing, or repeatedly failed visual, use [references/design-led-planner.md](references/design-led-planner.md) to compare two or three materially different causal hypotheses internally. Decide page purpose and responsibility, truthfulness/realism mode and evidence boundary, visual-quality mechanism, and a **Provisional Whole-Image Hypothesis** for each serious contender. This provisional whole is a short one-scene scaffold used to expose fragmentation before field work; it is not a brief, contract, prompt, or authority. Reject any hypothesis with one hard-factor conflict. Do not expose a weak option menu or create multiple Planner Briefs.
11. **Formula decision:** Audit the selected provisional whole through [references/visual-formula-brief.md](references/visual-formula-brief.md). Keep `T -> S -> R -> O -> A -> I -> N` as the complete semantic order; use the formula to correct unsupported or vague parts of the hypothesis, not to mechanically add components. Treat the three factors as cross-cutting checks, not new formula letters. Load only conditional modules whose observable trigger is present. Do not print empty or `not applicable` modules.
12. **Formal whole-image contract:** For a semantic new image, meaning-changing edit, high-impact visual, or repeated-failure reset, compile the audited Formula Decision into the formal Whole-Image Synthesis Contract inside the same Planner Brief. Require one sketchable scene, one core moment, one attention order, a bounded frame, three to six credibility anchors, and explicit `Fixed / Adaptive / Forbidden` freedom. The formal contract replaces the provisional scaffold; do not output both or turn either into a new formula authority.
13. **Reverse consistency check:** Re-read the formal contract against `T/S/R/O/A/I/N`, image/text/layout ownership, the three factor contracts, the strongest `N` misread, and any Scene Invariant Trace. Return when the mother statement drops a required decision, introduces an unsupported object/story, makes the page depend on rescue copy, or describes a different image from the formula.
14. **Composition readability:** Make `R` and `O` concrete enough that a person could roughly sketch the visible roles, relationship, reveal order, hierarchy, and useful copy space. Keep pixel-level method choices with imagegen.
15. **Production handoff:** Compile the selected and reverse-checked solution into one prompt-ready Formula Decision with the formal Whole-Image Synthesis Contract, protected content, QA checks, and rejection triggers inside the same Planner Brief. State the required realism and quality ceiling, but leave the production model, renderer, compositing path, prompt technique, and other pixel methods to imagegen unless the user supplied a method constraint.
16. **Semantic compatibility and route:** Reuse `Feasibility` to test `T-S` compatibility, the fact/material basis of image-owned `S/R`, the strongest plausible `N` misread, dependent formula fields, whole-image contract consistency, whether named HTML/layout owners support rather than rescue the image, and whether every required factor contract is `defined / executable / no known conflict`. End with exactly `PROCEED` or `RETURN`. `PROCEED` means the unrendered production contract is executable; it is not a result pass or user production authorization. `RETURN` forbids a production brief, prompt, or imagegen handoff.
...
Use [references/visual-formula-brief.md](references/visual-formula-brief.md) as the single imagegen handoff. The internal order is `design judgment and three-factor search -> Provisional Whole-Image Hypothesis -> formula audit -> Formal whole-image contract -> Reverse consistency check -> PROCEED / RETURN`. On `PROCEED`, output only the full core, the formal triggered Whole-Image Synthesis Contract, and triggered conditional modules; on `RETURN`, stop after the blocker record. Do not expose the provisional scaffold, repeat decisions, or imply rendered success. List only materially used formal files; imagegen owns production and pixel execution.
