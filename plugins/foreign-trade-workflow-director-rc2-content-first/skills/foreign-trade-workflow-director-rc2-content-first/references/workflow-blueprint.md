...
`blueprint_id: foreign-trade-complete-workflow-rc2-content-first-candidate`

`blueprint_version: 0.3.0-rc2-content-first.1`
...
`semantic_evaluation_mode` 只能是 `strict_audit` 或 `content_first`。缺省或历史 beta.3 合同按 `strict_audit`。`content_first` 必须来自候选地图技能的最终冻结合同，且每个评分对象必须有完整原始回答、可见输入哈希、来源真值对照、逐项评分、未知项和独立 `platform_audit_state`。
...
`CONTENT_CALIBRATION_PASS` 只能证明候选内容门已通过；它不是 beta.3 `EFFECTIVE`，也不能让 `industry_semantic_expansion` 变为 `PASS`。全量默认 `NOT_AUTHORIZED`；只有单独的授权引用、冻结的末端范围哈希和零安全失败才允许 `AUTHORIZED_NOT_STARTED`。全量、证据展开和反向审计完成后仍保持 `RESEARCH_ONLY_BLOCKED`：不得写共享应用底座、公司匹配、路线包、候选客户或对外沟通。
...
company_workflow_state:
  ...
  semantic_method:
    semantic_evaluation_mode: strict_audit | content_first
