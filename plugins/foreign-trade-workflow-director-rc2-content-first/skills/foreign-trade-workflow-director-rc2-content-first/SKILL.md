---
name: foreign-trade-workflow-director-rc2-content-first
description: Use when a company needs the isolated RC2 content-first candidate controller or beta.3 strict-audit compatibility to audit, resume, or reproduce the foreign-trade workflow while enforcing semantic content evidence, full-scope authorization, and downstream stopping points.
---

# 外贸全流程控制与复刻

## Candidate boundary and semantic mode

This candidate controller does not replace beta.3. Treat `strict_audit` as the default for a legacy contract with no mode field, and preserve beta.3 `INCONCLUSIVE / EFFECTIVE / NOT_EFFECTIVE` semantics exactly. Accept `content_first` only from the candidate map skill's explicit final frozen contract.

For `content_first`, the controller reads a content envelope, unchanged raw-response hash, visible-input hash, source/truth comparison, itemized scorecard, unknown items, method arm, and separate `platform_audit_state`. It must not use platform run ID, platform time, or strong model-identity evidence as a content PASS prerequisite. It must also not delete the raw response merely because platform audit is unavailable.

`CONTENT_CALIBRATION_PASS` is not beta.3 `EFFECTIVE` and does not make `industry_semantic_expansion` PASS. It yields one next action only: request or check explicit full-scope authorization. `RESEARCH_ONLY_BLOCKED` remains in force after any content-first result: shared-base writes, company matching, route handoff, candidate work, customer work, and sending are prohibited.
...
No controller action may change `RESEARCH_ONLY_BLOCKED` to a downstream PASS. A future decision to bridge candidate research into the official workflow requires a separately authored migration contract and user authorization; this candidate deliberately does not implement one.
