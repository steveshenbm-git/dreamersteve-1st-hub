...
Within `content_first`, source version `0.4.0-beta.4` uses the `content_first_r4` marker plus `case_package_contract_version = 1.0-beta4`. It remains the same `industry-application-map-builder` skill; do not create or route to a candidate skill. Never downgrade an R4 artifact because a marker, version or arm field is missing: that is a refusal, not legacy detection. Beta.3 R4 artifacts are historical read-only inputs and cannot be continued by a beta.4 lock or finalizer.
