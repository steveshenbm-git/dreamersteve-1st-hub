---
name: industry-application-map-builder-rc2-content-first
description: Use when an RC2 industry-semantic task needs the beta.3 strict-audit lane or the isolated content-first candidate lane, including 40-case scoring, full-scope authorization gates, source evidence, reverse audit, company mapping, route candidates, or controlled handoff before customer-development validation.
---

# 行业应用地图构建

## Candidate boundary and mode selection

This is an independent RC2 candidate, not a replacement for `industry-application-map-builder` beta.3. Do not edit, relabel, or merge beta.3, installed-cache, or frozen RC2-40 artifacts. Select exactly one semantic evaluation mode before a semantic route:

| Mode | Contract selection | Meaning | Status vocabulary |
|---|---|---|---|
| `strict_audit` | Explicit `execution_mode = strict_audit`, or absent in a legacy beta.3 contract | Use every beta.3 identity, transport, receipt, and admissibility gate unchanged | `INCONCLUSIVE / EFFECTIVE / NOT_EFFECTIVE` |
| `content_first` | Explicit candidate content-first final frozen contract | Score content from preserved raw answers and source/truth evidence; record platform audit separately | `CONTENT_CALIBRATION_INCOMPLETE / CONTENT_CALIBRATION_PASS / CONTENT_CALIBRATION_FAIL` |

Never infer `content_first` from a missing receipt or unknown model metadata. A legacy contract without `execution_mode` remains `strict_audit`. A content-first result is research-only and must never be written as beta.3 `EFFECTIVE`, a shared-base fact, a company map, a route, a customer result, or a downstream authorization.

Read [content-first-mode-contract.md](references/content-first-mode-contract.md) and [compatibility-matrix.md](references/compatibility-matrix.md) before a content-first route, in addition to the three RC2 semantic references. Use the candidate templates in `assets/content-first/`; do not retrofit their fields into a frozen beta.3 contract.
...
| `content_first_contract_prepare` | Freeze a candidate content-first contract and rubric after the existing preparation/case-set gates | [content-first-mode-contract.md](references/content-first-mode-contract.md) | `CONTENT_CONTRACT_FROZEN`; stop before 40-case content work |
| `content_first_calibration_review` | Score both 40-case arms from raw-answer envelopes and source/truth packets | [content-first-mode-contract.md](references/content-first-mode-contract.md) | `CONTENT_CALIBRATION_*`; stop at full-scope authorization request |
...
For `content_first`, do not use or modify `method_validation_state`. Keep `content_method_state = CONTENT_CALIBRATION_INCOMPLETE` until the candidate 40-case evidence gate passes. This is not a claim of real-world method effectiveness: it only determines whether an explicit full-scope authorization may be requested in this candidate research lane.
